...
const chartMarkup = {
  bachelier: () => {
    const paths = Array.from({ length: 4 }, (_, pathIndex) => {
      ...
      for (let x = 60; x <= 600; x += 20) {
        const wave = Math.sin(x * 0.071 + pathIndex * 1.7) * 9;
        const drift = (pathIndex - 1.5) * 0.12;
        y = Math.max(35, Math.min(265, y + wave + drift));
  ...
  option: () => `<line class="chart-axis" x1="55" y1="255" x2="610" y2="255"/><line class="chart-axis" x1="55" y1="35" x2="55" y2="255"/><line class="chart-dashed" x1="360" y1="35" x2="360" y2="255"/><line class="chart-dashed" x1="55" y1="255" x2="610" y2="255"/><polyline class="chart-line" points="55,255 360,255 600,55"/><text class="chart-label" x="565" y="275">S<tspan baseline-shift="sub">T</tspan></text><text class="chart-label" x="365" y="275">K = 100</text><text class="chart-label" x="64" y="30">PAYOFF</text><text class="chart-label dark" x="458" y="74">max(S<tspan baseline-shift="sub">T</tspan> − K, 0)</text>`,
...
document.querySelectorAll(".event-trigger").forEach((trigger) => {
